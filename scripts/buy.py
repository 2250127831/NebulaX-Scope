#!/usr/bin/env python3
"""
Send N NEW BUY at a given price, with optional rate limit.
Usage:  python3 buy.py <port> <price> [count] [rate]
...
def cmd_new(price: int, uid: int) -> bytes:
    return struct.pack("<BBxxII4xQQ", 1, 0x01, price, 10, uid, 0)
...
if RATE > 0:
    print(f"BUY {N} @ {PRICE}  rate={RATE}/s …", flush=True)
else:
    print(f"BUY {N} @ {PRICE} …", flush=True)
t0 = time.time()
